...
bool findNewOpenCell(cell *cell_){
    if(maze[cell_->get_coordy()][cell_->get_coordx()].get_type() == CELL::BORDER) return false; // the cell is a border
...
void includeCell(cell* cell_, cell* parent_cell){
    ...
    if(cell_->get_type() != CELL::START_CELL && cell_->get_type() != CELL::END_CELL){
        cell_->setType(CELL::OPEN_CELL);
    }
...
void CheckSurroundingCells(cell* parent_cell){
    cell* topRightCell =    &maze[parent_cell->get_coordy() + 1][parent_cell->get_coordx() - 1];
    cell* rightCell =       &maze[parent_cell->get_coordy() + 1][parent_cell->get_coordx()    ];
    cell* bottomRightCell = &maze[parent_cell->get_coordy() + 1][parent_cell->get_coordx() + 1];
    cell* bottomCell =      &maze[parent_cell->get_coordy()    ][parent_cell->get_coordx() + 1];
    cell* bottomLeftCell =  &maze[parent_cell->get_coordy() - 1][parent_cell->get_coordx() + 1];
    cell* leftCell =        &maze[parent_cell->get_coordy() - 1][parent_cell->get_coordx()    ];
    cell* topLeftCell =     &maze[parent_cell->get_coordy() - 1][parent_cell->get_coordx() - 1];
    cell* topCell =         &maze[parent_cell->get_coordy()    ][parent_cell->get_coordx() - 1];
...
std::vector<cell*> AstarPathfinding(cell *start_cell_, cell *end_cell_){
    ...
    start_cell_->setType(CELL::START_CELL);
    end_cell_->setType(CELL::END_CELL);
    ...
    while(1){
        ...
        if (chosen.empty() || current != chosen.back()){
            ...
            if(current->get_type() != CELL::START_CELL && current->get_type() != CELL::END_CELL){
                current->setType(CELL::CHOSEN_CELL);
            }
        }
        else { // we are blocked
        ...
        if(current->get_coordx() == end_cell_->get_coordx() && current->get_coordy() == end_cell_->get_coordy()){
            ...
            while(current_->get_coordx() != start_cell_->get_coordx() && current_->get_coordy() != start_cell_->get_coordy()){
                ...
                if(current_->getParentCell()->get_type() != CELL::START_CELL) current_->getParentCell()->setType(CELL::PATH);
